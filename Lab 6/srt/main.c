...
void * multithreader(void * number){
    int num = *(int*)number;
    ...
    for( int px = num; px < width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            map[px][py][0] = scaled_color[0];
            map[px][py][1] = scaled_color[1];
            map[px][py][2] = scaled_color[2];
        }
    }
...
main( int argc, char **argv )
{
    ...
    pthread_t *AllThreads = (pthread_t*)malloc(sizeof(pthread_t) * nthreads);
    if (AllThreads == NULL){
    ...
    for (int n = 0; n < nthreads; n++ ){
        IDs[n] = n;
        if(pthread_create(&AllThreads[n], NULL, multithreader, &IDs[n]))
        {
    ...
    for( int n = 0; n < nthreads; n++ ){
        if( pthread_join( AllThreads[n], NULL )){
